...
namespace winrt::WindowsSample::implementation
{
    ...
    HRESULT SimpleMediaSource::Initialize(_In_ IMFAttributes* pAttributes)
    {
        ...
        wil::unique_cotaskmem_string mappingName;
        wil::unique_cotaskmem_string userSid;
        UINT32 mappingLength = 0;
        UINT32 sidLength = 0;
        RETURN_IF_FAILED(m_spAttributes->GetAllocatedString(
            ...
            &mappingLength));
        RETURN_IF_FAILED(m_spAttributes->GetAllocatedString(
            PROJECTBLUR_FRAME_USER_SID,
            &userSid,
            &sidLength));
        RETURN_HR_IF(E_INVALIDARG, mappingLength == 0 || sidLength == 0);
        m_frameMappingName.assign(mappingName.get(), mappingLength);
        m_frameUserSid.assign(userSid.get(), sidLength);
