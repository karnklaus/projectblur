...
namespace
{
        ...
        IMFMediaType** mediaType)
    {
        ...
        *mediaType = nullptr;
        ...
        RETURN_IF_FAILED(type->SetUINT32(
        ...
        RETURN_IF_FAILED(MFSetAttributeRatio(
            type.get(), MF_MT_FRAME_RATE, 30, 1));
        RETURN_IF_FAILED(MFSetAttributeRatio(
        ...
        RETURN_IF_FAILED(type->SetUINT32(
            MF_MT_AVG_BITRATE,
            static_cast<UINT32>(bytesPerFrame * 8U * 30U)));
        return type.copy_to(mediaType);
    }
}
...
namespace winrt::WindowsSample::implementation
{
        ...
        )
    {
        ...
        m_parent = pSource;
        m_frameMappingName = pSource->FrameMappingName();
        m_frameUserSid = pSource->FrameUserSid();
        ...
        RETURN_IF_FAILED(CreateVideoMediaType(
            MFVideoFormat_NV12, 1280, 720, &mediaTypeList[0]));
        RETURN_IF_FAILED(CreateVideoMediaType(
            MFVideoFormat_RGB32, 1280, 720, &mediaTypeList[1]));
        RETURN_IF_FAILED(CreateVideoMediaType(
            MFVideoFormat_NV12, 1920, 1080, &mediaTypeList[2]));
        RETURN_IF_FAILED(CreateVideoMediaType(
            MFVideoFormat_RGB32, 1920, 1080, &mediaTypeList[3]));
        ...
        )
    {
        ...
        const LONGLONG beforeWait = MFGetSystemTime();
        if (m_nextSampleTime > beforeWait)
        {
            const LONGLONG remaining = m_nextSampleTime - beforeWait;
            LARGE_INTEGER dueTime;
            dueTime.QuadPart = -remaining;
            RETURN_HR_IF_NULL(E_UNEXPECTED, m_pacingTimer.get());
            RETURN_IF_WIN32_BOOL_FALSE(SetWaitableTimerEx(
                m_pacingTimer.get(),
                &dueTime,
                0,
                nullptr,
        ...
        if (m_nextSampleTime > 0 &&
            sampleTime - m_nextSampleTime <= m_frameDuration)
        {
            sampleTime = m_nextSampleTime;
        }
        ...
        RETURN_IF_FAILED(sample->SetUINT64(
            PROJECTBLUR_SAMPLE_SEQUENCE,
            metadata.sequence));
        RETURN_IF_FAILED(sample->SetUINT64(
            PROJECTBLUR_SAMPLE_SOURCE_TIMESTAMP_NS,
            metadata.source_timestamp_ns));
        RETURN_IF_FAILED(sample->SetUINT32(
            PROJECTBLUR_SAMPLE_FLAGS,
            metadata.flags));
        if (pToken != nullptr)
        {
    ...
    HRESULT SimpleMediaStream::StartInternal(bool bSendEvent, IMFMediaType* pNewMediaType)
    {
        ...
        if ((m_streamState != MF_STREAM_STATE_RUNNING) || !bMatch)
        {
            ...
            if (!m_pacingTimer)
            {
                ...
                RETURN_LAST_ERROR_IF_NULL(m_pacingTimer.get());
            }
